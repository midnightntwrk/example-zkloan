...
import { transientHash, CompactTypeBytes } from '@midnight-ntwrk/compact-runtime';
import { ZKLoanCreditScorer, witnesses } from 'zkloan-credit-scorer-contract';
...
export const getZKLoanLedgerState = async (providers, contractAddress) => {
...
export const zkLoanCompiledContract = CompiledContract.make('ZKLoanCreditScorer', ZKLoanCreditScorer.Contract).pipe(CompiledContract.withWitnesses(witnesses), CompiledContract.withCompiledFileAssets(contractConfig.zkConfigPath));
...
export const computeUserPubKeyHash = (zwapKeyBytes, pin) => {
    const pubKey = pureCircuits.publicKey(zwapKeyBytes, pin);
    return transientHash(bytes32Type, pubKey);
};
export const fetchAttestation = async (attestationApiUrl, creditScore, monthlyIncome, monthsAsCustomer, userPubKeyHash) => {
...
export const requestLoan = async (contract, providers, amountRequested, secretPin, zwapKeyBytes, attestationApiUrl) => {
    ...
    const signature = await fetchAttestation(attestationApiUrl, Number(currentState.creditScore), Number(currentState.monthlyIncome), Number(currentState.monthsAsCustomer), userPubKeyHash);
    // 4. Get provider info
...
export const removeProvider = async (contract, providerId) => {
...
export const displayContractState = async (providers, contract) => {
...
}), Rx.filter((state) => state.isSynced), Rx.map((s) => (s.unshielded?.balances[ledger.nativeToken().raw] ?? 0n) + (s.shielded?.balances[ledger.nativeToken().raw] ?? 0n)), Rx.filter((balance) => balance > 0n)));
/**
 * Display wallet balances (unshielded, shielded, total)
 */
export const displayWalletBalances = async (wallet) => {
 ...
 */
export const registerNightForDust = async (walletContext) => {
    ...
    const unregisteredNightUtxos = state.unshielded?.availableCoins.filter((coin) => coin.meta.registeredForDustGeneration === false) ?? [];
    ...
    try {
        const recipe = await walletContext.wallet.registerNightUtxosForDustGeneration(unregisteredNightUtxos, walletContext.unshieldedKeystore.getPublicKey(), (payload) => walletContext.unshieldedKeystore.signData(payload));
        ...
        await Rx.firstValueFrom(walletContext.wallet.state().pipe(Rx.throttleTime(5_000), Rx.tap((s) => {
 ...
 */
export const initWalletWithSeed = async (seed, config) => {
 ...
 */
export const buildWalletAndWaitForFunds = async (config, mnemonic) => {
 ...
 */
export const buildWalletFromHexSeed = async (config, hexSeed) => {
...
export const closeWallet = async (walletContext) => {
    try {
