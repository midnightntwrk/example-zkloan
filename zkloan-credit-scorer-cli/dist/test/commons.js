...
import { StandaloneConfig, currentDir, TestnetRemoteConfig } from '../config';
...
export class TestEnvironment {
    ...
    start = async () => {
        if (process.env.RUN_ENV_TESTS === 'true') {
            ...
            this.dockerEnv = new DockerComposeEnvironment(path.resolve(currentDir, '..'), composeFile)
                .withWaitStrategy('zkloan-proof-server', Wait.forLogMessage('Actix runtime found; starting in Actix runtime', 1))
            ...
            this.testConfig.dappConfig = {
                ...
                proofServer: TestEnvironment.mapContainerPort(this.env, this.testConfig.dappConfig.proofServer, 'zkloan-proof-server'),
            };
        }
    ...
    static getProofServerContainer = async (env) => await new GenericContainer('midnightnetwork/proof-server:4.0.0')
