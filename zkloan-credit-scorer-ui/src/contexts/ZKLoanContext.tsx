  ...
  const initializeProviders = useCallback(async () => {
    ...
    const midnightProvider: MidnightProvider = {
      async submitTx(tx: ledger.FinalizedTransaction): Promise<ledger.TransactionId> {
        ...
        const serialized = tx.serialize();
        const serializedStr = uint8ArrayToHex(serialized);
        // Pass {} as second arg for the same reason as balanceUnsealedTransaction:
        // Lace's messaging layer appends {sender} as the last arg, so we need
        // an options placeholder to keep the args aligned.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await (wallet as any).submitTransaction(serializedStr, {});
